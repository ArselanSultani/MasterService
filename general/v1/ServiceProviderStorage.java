...
public class ServiceProviderStorage  implements ServiceInterface {
    ...
    class StorageArea implements ServiceObject{
        ...
        void sortArea(){
            List<CachebleObj> li = new ArrayList<>(areaData.values());
            Collections.sort(li, new Comparator<CachebleObj>() {
                @Override
                public int compare(CachebleObj t1, CachebleObj t2) {
                    return t1.getLatestAccessedTime().compareTo(t2.getLatestAccessedTime());
                }
            });

            areaData = li.stream().collect(Collectors.toMap(CachebleObj::getId, it -> it));
        }

        public CachebleObj getObject(String name){
            CachebleObj cachebleObj =null;
            lock.lock();
            try {
                for(Map.Entry<Integer, CachebleObj> singleObject: areaData.entrySet()) {
                    if(singleObject.getValue().getObjectName().equals(name)){
                        cachebleObj =  singleObject.getValue();
                        break;
                    }
                }
                sortArea();
